%Exam 2012 made by Martin Iriondo
clc
clear all 
close all
%Examen 2012 optimizacion -- ejercicio 1 

%ejercicio 1:
syms x1 x2 v1
f=(x1-1)^2+(x2-1)^2;%ecuación de partida    ****
h1=x1+x2-4;%=0 constraint     ****
%L=f+v1*h1+v2*h2+u1*(g1+s1^2)+u2*(g2+s2^2);
L=f+v1*h1
KKT=    [diff(L,x1); 
         diff(L,x2);
         diff(L,v1)]
...
[x] = Exer4_45_func2012()  % Get the solution

[x1,x2] = meshgrid(-2:0.05:6, -2.0:0.05:4); % % Create a grid 
% Compute the functions for the minimization problem
f = (x1 - 1).^2 + (x2 - 1).^2; % Important: element-by-element operation indicated by "dot": . CAMBIAR
h1 = x1 + x2 - 4;%CAMBIAR
...
title ('EXAM 2012') % Display a title for the problem
